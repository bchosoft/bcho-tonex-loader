...
const (
	...
	appDataDirName = "BCho_TONEX_Loader" // folder under the user config dir
	tokenPrefix    = "LDR"               // prefix of the token pasted in the donation
	tokenHexLen    = 10
...
// ApplyCode validates a manually generated unlock code against this machine and stores it.
func (a *App) ApplyCode(code string) (bool, error) {
	...
	if code == "" {
		return false, errors.New("codigo vacio")
	}
	if !verifyLicense(h, code) {
		return false, errors.New("el codigo no es valido para este equipo")
	}
...
// CheckDonation asks the server whether this machine's token has paid.
func (a *App) CheckDonation() (bool, error) {
	...
	if err := json.Unmarshal(body, &r); err != nil {
		return false, errors.New("respuesta del servidor no valida")
	}
	if !r.Paid {
		return false, nil
	}
	if r.License == "" || !verifyLicense(h, r.License) {
		return false, errors.New("la licencia recibida no es valida")
	}
...
func (a *App) checkImportAllowed() error {
	cfg := a.GetMonetizationConfig()
	if !cfg.Restrictions {
		return nil
	}
	...
	if a.importsDone >= limit {
		...
		return errDonationLimit
	}
	return nil
}

func (a *App) addImport() {
	cfg := a.GetMonetizationConfig()
	if !cfg.Restrictions {
		return
	}
